...
// src/api/apiChat.ts
var apiChat = axios_default.create({
...
// src/services/name-service.ts
var sendQuestionService = (bodyValue, res) => __async(void 0, null, function* () {
  var resposta = null;
  try {
    const apiResponse = yield apiChat.post(
      "/chat",
      bodyValue,
      { responseType: "stream" }
    );
    ...
    const logStream = new import_stream5.Transform({
      transform(chunk, encoding, callback) {
        callback(null, chunk);
      }
    });
    apiResponse.data.pipe(logStream).pipe(res);
  } catch (error) {
...
var sendQuestion = (req, res) => __async(void 0, null, function* () {
  const bodyValue = req.body;
  yield sendQuestionService(bodyValue, res);
});
